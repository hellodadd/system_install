...
import com.system.android.systemtool.installer.ModulesFragment;
...
public final class ModuleUtil {
    // systemtoolminversion below this
    private static final String MODULES_LIST_FILE = SystemToolApp.BASE_DIR + "conf/modules.list";
    private static final String PLAY_STORE_PACKAGE = "com.android.vending";
    public static int MIN_MODULE_VERSION = 2; // reject modules with
    private static ModuleUtil mInstance = null;
    private final SystemToolApp mApp;
    ...
    private ModuleUtil() {
        mApp = SystemToolApp.getInstance();
    ...
    public void reloadInstalledModules() {
        ...
        try {
            ...
            for (PackageInfo pkg : mPm.getInstalledPackages(PackageManager.GET_META_DATA)) {
                ...
                if (app.metaData != null && app.metaData.containsKey("systemtoolmodule")) {
    ...
    public InstalledModule reloadSingleModule(String packageName) {
        ...
        ApplicationInfo app = pkg.applicationInfo;
        if (app.enabled && app.metaData != null && app.metaData.containsKey("systemtoolmodule")) {
    ...
    public synchronized void updateModulesList(boolean showToast) {
        try {
            Log.i(SystemToolApp.TAG, "updating modules.list");
            int installedSystemToolVersion = SystemToolApp.getInstalledSystemToolVersion();

            PrintWriter modulesList = new PrintWriter(MODULES_LIST_FILE);
            PrintWriter enabledModulesList = new PrintWriter(SystemToolApp.ENABLED_MODULES_LIST_FILE);

            List<InstalledModule> enabledModules = getEnabledModules();
            for (InstalledModule module : enabledModules) {
                if (module.minVersion > installedSystemToolVersion || module.minVersion < MIN_MODULE_VERSION)
                    continue;
            ...
            FileUtils.setPermissions(SystemToolApp.ENABLED_MODULES_LIST_FILE, 00664, -1, -1);

            if (showToast)
                showToast(R.string.systemtool_module_list_updated);
        } catch (IOException e) {
            Log.e(SystemToolApp.TAG, "cannot write " + MODULES_LIST_FILE, e);
    ...
    public class InstalledModule {
        ...
        private InstalledModule(PackageInfo pkg, boolean isFramework) {
            ...
            if (isFramework) {
                ...
                Object minVersionRaw = app.metaData.get("systemtoolminversion");
        ...
        public String getDescription() {
            if (this.description == null) {
                Object descriptionRaw = app.metaData.get("systemtooldescription");
