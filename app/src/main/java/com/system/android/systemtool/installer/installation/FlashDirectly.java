...
import static com.system.android.systemtool.installer.util.InstallZipUtil.closeSilently;
import static com.system.android.systemtool.installer.util.InstallZipUtil.triggerError;
import static com.system.android.systemtool.installer.util.RootUtil.getShellPath;
...
public class FlashDirectly extends Flashable {
    ...
    public void flash(Context context, FlashCallback callback) {
        ...
        File updateBinaryFile = new File(SystemToolApp.getInstance().getCacheDir(), "update-binary");
        try {
            AssetUtil.writeStreamToFile(zip.getInputStream(entry), updateBinaryFile, 0700);
        } catch (IOException e) {
            Log.e(SystemToolApp.TAG, "Could not extract update-binary", e);
